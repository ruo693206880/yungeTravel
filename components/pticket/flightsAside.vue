<template>
	<div class="aside">
		...
		<div class="history">
			<h5>历史查询</h5>
			<nuxt-link v-for="(item, index) in $store.state.pticket.history" :key="index" :to="`/pticket/flights?departCity=${item.departCity}&departCode=${item.departCode}&destCity=${item.destCity}&destCode=${item.destCode}&departDate=${item.departDate}`">
